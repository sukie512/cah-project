  ...
  return (
    <>
      ...
      <section className="flex flex-col items-center text-center px-4 pt-16 pb-8">
        ...
        <blockquote className="text-lg md:text-xl italic mb-1">
          {page.hero.quote}
        </blockquote>
        <cite className="text-sm not-italic text-gray-600 mb-8">
          {page.hero.quoteSource}
        </cite>

        {/* Icon badges marquee */}
        <div className="w-full overflow-hidden mb-8">
          <div className="marquee-track flex gap-4 w-max">
            {[...page.hero.badgeIcons, ...page.hero.badgeIcons].map((icon, i) => (
              icon.url ? (
                <Image
                  key={i}
                  src={icon.url}
                ...
                <div key={i} className="w-9 h-9 bg-black rounded-full flex-shrink-0 opacity-20" />
              )
            ))}
          </div>
        </div>
        ...
        <p className="max-w-xl text-base md:text-lg leading-relaxed mb-4">
          <strong>{page.hero.tagline}</strong> {page.hero.description.replace(page.hero.tagline, '').trim()}
        </p>
      ...
      <section className="px-4 md:px-8 py-12 relative">
        ...
        <p className="max-w-2xl text-sm md:text-base leading-relaxed mb-8">
          {page.steal.body}{' '}
          <a
            href={page.steal.downloadUrl}
            target="_blank"
        ...
        <div className="flex items-center gap-6">
          <a
            href={page.steal.downloadUrl}
            className="bg-black text-white font-bold uppercase text-sm px-6 py-3 hover:bg-gray-900 transition-colors"
            target="_blank"
            rel="noopener noreferrer"
          >
            Download Files
        ...
        <div className="max-w-lg">
          ...
          <form onSubmit={(e) => e.preventDefault()} className="flex gap-0">
            <input
              ...
              className="flex-1 border border-black px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-black"
            />
            <button
              type="submit"
              className="bg-black text-white text-xs font-bold uppercase px-5 py-3 hover:bg-gray-900 transition-colors"
            >
              Submit
            </button>
          </form>
